...
function init()
{
	...
	var lutes = getID('optLute').checked;
	var sortTypes = 0;
	...
	for (let i = 0; i < ary_SongData.length; i++) {
		...
		if (anyIntersection(ary_SongData[i][TRACK_TITLES], selectionSet)) {
			...
			const correctLuteType = lutes || (ary_SongData[i][TRACK_IS_ARRANGEMENT] === ORIGINAL_TRACK);

			if (correctTrackType && correctArrangementType && correctLuteType) {
